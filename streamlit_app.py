import streamlit as stimport osimport warningsimport timefrom datetime import datetimeimport iofrom io import BytesIOwarnings.filterwarnings('ignore')
FIX FOR CHROMADB/SQLITE3
try:    import sys    import pysqlite3    sys.modules['sqlite3'] = sys.modules.pop('pysqlite3')    import sqlite3    st.info(f"✅ SQLite patched to version: {sqlite3.sqlite_version}")except ImportError:    st.error("❌ pysqlite3-binary not installed. Add to requirements.txt.")
Import Required Libraries
from crewai import Agent, Task, Crew, Processfrom crewai.llm import LLMimport json
DOCX Libraries for Word Document Generation
from docx import Documentfrom docx.shared import Inches, Ptfrom docx.enum.text import WD_ALIGN_PARAGRAPHfrom docx.enum.style import WD_STYLE_TYPEfrom docx.oxml.shared import OxmlElement, qn
Fix RGB Color import
try:    from docx.shared import RGBColorexcept ImportError:    pass  # Use default colors
Configure Streamlit Page
st.set_page_config(    page_title="Multi-Agent AI Analysis System",    page_icon="🤖",    layout="wide")
...
@st.cache_resourcedef get_groq_llm(api_key):    return LLM(        model="groq/llama-3.1-8b-instant",        api_key=api_key,        max_tokens=1000,        temperature=0.1    )
STEP 5: Professional Document Formatter Class (adapted for Streamlit)
class MultiAgentDocumentFormatter:    """    Professional Word Document Formatter for Multi-Agent Analysis Results    Creates executive-level reports with proper formatting and structure    """
def __init__(self):
...
@st.cache_resourcedef create_agents(api_key):    groq_llm = get_groq_llm(api_key)
# RESEARCH AGENT
...
def create_research_task(user_query):    return Task(        description=f"""        Conduct comprehensive research and analysis on: "{user_query}"
...
def create_analysis_task(user_query, research_results=None):    research_context = f"\n\nRESEARCH CONTEXT:\n{research_results}" if research_results else ""
return Task(
    description=f"""
...
def create_orchestration_task(user_query, research_data, analysis_data):    return Task(        description=f"""        Create an executive-level synthesis report for: "{user_query}"
...
def run_hybrid_multi_agent_analysis_with_docx(user_query, api_key, export_to_docx=True):    """    Execute complete hybrid multi-agent workflow with professional DOCX export    Returns results dict and optional docx bytes    """
...
def create_docx_bytes(results):    """    Generate a professional Word document from multi-agent analysis results    Returns bytes for download    """
try:
...
Research Agent: Senior Research Specialist with domain expertiseAnalysis Agent: Senior Data Analyst and Strategic AdvisorOrchestrator Agent: Chief Synthesis Expert and Executive AdvisorLLM Technology: Groq Llama-3.1-8B-InstantProcessing Method: Sequential multi-phase analysis with rate limitingDocument Generated: {results['timestamp']}Total Processing Time: ~3-4 minutes including safety delays"""
...
Main Streamlit App
def main():    st.markdown('🤖 Multi-Agent AI Analysis System', unsafe_allow_html=True)    st.markdown('Professional Intelligence Analysis with Executive Reporting', unsafe_allow_html=True)
# Sidebar for API Key
